...
func runner_go() string {
	sb := strings.Builder{}
	sb.WriteString("// file generated by Boa Constructor from input by " + appData.author.Text + "\n\n")
...
func Runcli(w io.Writer) int {
	...
	if cli == nil {
		w.Write([]byte("unable to parse input"))
		return -1
	}
	`)
...
`
	...
	for _, c := range cmdInfo.commands {
		...
		w.Write([]byte("` + c.Name + ` not implemented"))
	}
	`
		} else {
			implStr = ` {
